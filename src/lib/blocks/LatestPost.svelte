...
<section class="flex flex-col bg-light py-14 md:mt-12 px-sm-padding md:px-md-padding xl:px-lg-padding max-w-content">
  ...
  <div class="flex flex-col-reverse md:flex-row">
    <div class="flex flex-col md:w-1/2 md:pr-24">
      ...
      <span>{summary}</span>
      <a href={seo.slug.current} class="font-bold hover:text-blue mt-6 flex items-center gap-1 hover:gap-2 duration-200">
        {linkText}
        <IconChevronRight />
      </a>
    </div>

    <a href={seo.slug.current} class="relative md:w-1/2 group">
